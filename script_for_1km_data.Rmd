...
ggplot(clim_m_test[reg=="DAS"],aes(x=date,y=rain_med_dif,fill=season))+
  geom_col()+
  facet_grid(Summit~season)
ggplot(clim_med_per[reg=="DAS"],aes(x=month,y=wsup_med,fill=period))+
...
ggplot(clim_m_test[reg=="DAS"],aes(x=date,y=wsup_dif,fill=period))+
  geom_col()+
  facet_grid(Summit~.)+
  scale_x_date( breaks =date_breaks("2 years"),date_labels = "%Y")+
  labs(x="Year",y="Monthly anomalies of water supply (mm)",fill="Monitoring cycle",title="(a) DAS")+
  scale_fill_discrete(labels = c("2005-2009", "2010-2014", "2015-2019"))
ggsave("E:/Google 雲端硬碟/GLORIA_個人處理/paper_準備/SYU_DAS_water/result/plot/DAS_monthly_ws.jpeg",height=5,width = 10,dpi=300)
fwrite(clim_m_test,file="E:/Google 雲端硬碟/GLORIA_個人處理/paper_準備/SYU_DAS_water/result/monthly_precipitation_watersup.csv")
