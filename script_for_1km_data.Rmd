...
library(broom) #extract the result of lm
...
sv_path <- "G:/我的雲端硬碟/GLORIA_個人處理/paper_準備/SYU_DAS_water/"
...
for(i in s_p[reg %in% c("DAS","SYU"),Summit]){
...
ggsave(paste0(ld_path,"/result/plot/w_sup/monthly_",i,"_watersup_.jpeg"),plot = pr,height=8,width = 12,dpi=300)
}
```
### step 6. 嘗試表現水分減少
想法1，利用長期平均與正負差值來表現
...
fwrite(clim_m_test,file="G:/我的雲端硬碟/GLORIA_個人處理/paper_準備/SYU_DAS_water/result/monthly_precipitation_watersup.csv")
fwrite(summit_clim_avg,file="G:/我的雲端硬碟/GLORIA_個人處理/paper_準備/SYU_DAS_water/result/Summit_avg_T_rain.csv")
...
ggsave(paste0(sv_path,"result/plot/DAS_SYU_monthly_ws_anomal.jpeg"),height=9,width = 8,dpi=600)
...
ggplot(clim_m_test,aes(x=date,y=w_sup,fill=period))+
  ...
  facet_grid(Summit~.)+
  scale_x_date( breaks =date_breaks("2 years"),date_labels = "%Y")+
  labs(x="Year",y="Monthly water supply (mm)",fill="Monitoring cycle")+
  scale_fill_discrete(labels = c("2005-2009", "2010-2014", "2015-2019"))+
  #theme_classic()+
  scale_fill_brewer(palette="Dark2")
ggsave(paste0(sv_path,"result/plot/DAS_SYU_monthly_ws.jpeg"),height=9,width = 8,dpi=600)
...
p <- ggplot(pe_y[reg==`i`],aes(x=year,y=p_e))+
  ...
  stat_poly_eq(formula = y ~ x,aes(label = paste(..eq.label..,..rr.label..,
                                                 ..p.value.label..,sep = "~~~~")),
               label.x.npc = "right",parse=TRUE,size = 3)+
  labs(x="Year",y="PE (mm)")
...
ggsave(paste0(sv_path,"result/plot/annual_p_e.jpeg"),height=8,width = 10,dpi=300)
##### water supply
